 ...
 */
export const FRICTION_SLIP_REAR = 0.95;
...
/**
 * Передняя ось держит почти намертво — это не оговорка.
 *
 * Пробовал отпустить и её «для льда»: машину перестало быть чем направлять, снос
 * наружу накладывался на срыв зада, и ездить стало нельзя. Управляемый занос — это
 * скользкий зад при цепком переде: перед задаёт направление, зад уходит в скольжение.
 */
export const SIDE_FRICTION_FRONT = 0.9;
/**
 ...
 */
export const SIDE_FRICTION_REAR = 0.12;
export const SIDE_FRICTION_REAR_TURN = 0.035;
...
/**
 * Скорость затухания «разрешения на занос», 1/с.
 *
 * Потолок заноса поднимается рулём, но опускается с задержкой. Это ключевое: на
 * клавиатуре стрелку постоянно отпускаешь и дожимаешь, и если бы потолок падал
 * мгновенно, занос гасился бы в каждую паузу между нажатиями — именно поэтому
 * машина раньше «почти не срывалась». Короткие отпускания разрешение не сбрасывают,
 * а выровненный на секунду руль — сбрасывает, и машина выходит на курс.
 */
export const DRIFT_PERMIT_DECAY = 1.2;
