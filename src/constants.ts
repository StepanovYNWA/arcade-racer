...
export const SIDE_FRICTION_REAR_TURN = 0.03;
...
/**
 * Скорость затухания сглаженного угла руля, рад/с.
 *
 * Сглаженный угол повторяет руль мгновенно при довороте и отстаёт при возврате.
 * По нему считается всё: падение сцепления, потолок заноса и целевая скорость
 * доворота. Это ключевое: на клавиатуре стрелку
 * постоянно отпускаешь и дожимаешь, и если бы и сцепление, и потолок возвращались
 * мгновенно, скольжение гасилось бы в каждую паузу между нажатиями — именно поэтому
 * машина «почти не срывалась». Короткие отпускания скольжение не прерывают,
 * а выровненный на секунду руль — прерывает, и машина выходит на курс.
 */
export const STEER_SMOOTH_DECAY = 0.2;
...
/**
 * Мёртвая зона руля для сцепления, доля выворота.
 *
 * Сцепление падает не от любого касания руля, а только начиная с этого выворота:
 * мелкие правки на прямой машину не срывают, а заметный поворот срывает.
 *
 * Считается от ТЕКУЩЕГО максимума угла колёс. Считать от полного STEER_MAX было
 * ошибкой: угол колёс урезается скоростью, на 25 м/с он достигает лишь 0.7 от
 * полного, на 35 м/с — 0.45, и скользкие значения на гоночных скоростях просто
 * не достигались. Колёса всегда оставались цепкими, отсюда и «дрифта нет».
 */
export const STEER_GRIP_DEADZONE = 0.3;
/**
 * Доля выворота, на которой срыв уже полный.
 *
 * Сцепление падает не до самого упора, а насыщается раньше. Иначе в паузу между
 * нажатиями сглаженный угол проседает с 1.0 до ~0.77, и при линейной зависимости
 * сцепление подскакивало бы в несколько раз — скольжение обрывалось бы на каждом
 * отпускании стрелки.
 */
export const STEER_GRIP_FULL = 0.72;
